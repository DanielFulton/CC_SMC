...
SMC is a Java application that translates a state transition table into a program that implements the described state machine.  Output languages include Java, C, GDScript, Swift and C++.  Adding other languages is trivial.

###Command Line
`java -jar smc.jar -l <language> -o <directory> -f <flags>`

 * `<language>` is `C`, `Cpp`, `GDScript`, `Swift` or `Java`.  
